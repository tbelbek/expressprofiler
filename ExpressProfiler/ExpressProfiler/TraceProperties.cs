...
namespace ExpressProfiler
{
    public partial class TraceProperties : Form
    {
        ...
        {
            Settings.Default.TraceSettings = m_currentsettings.GetAsXmlString();
        ...
        public bool IsIncluded(ListViewItem lvi)
        {
            ...
            included &= IsIncluded(m_currentsettings.Filters.ApplicationNameFilterCondition,
                m_currentsettings.Filters.ApplicationName, lvi.SubItems[0].Text);
            included &= IsIncluded(m_currentsettings.Filters.TextDataFilterCondition,
                m_currentsettings.Filters.TextData, lvi.SubItems[1].Text);
            included &= IsIncluded(m_currentsettings.Filters.LoginNameFilterCondition,
                m_currentsettings.Filters.LoginName, lvi.SubItems[2].Text);
            included &= IsIncluded(m_currentsettings.Filters.CpuFilterCondition, m_currentsettings.Filters.CPU,
                lvi.SubItems[3].Text);
            included &= IsIncluded(m_currentsettings.Filters.ReadsFilterCondition, m_currentsettings.Filters.Reads,
                lvi.SubItems[4].Text);
            included &= IsIncluded(m_currentsettings.Filters.WritesFilterCondition, m_currentsettings.Filters.Writes,
                lvi.SubItems[5].Text);
            included &= IsIncluded(m_currentsettings.Filters.DurationFilterCondition,
                m_currentsettings.Filters.Duration, lvi.SubItems[6].Text);
            included &= IsIncluded(m_currentsettings.Filters.SPIDFilterCondition, m_currentsettings.Filters.SPID,
                lvi.SubItems[7].Text);
        ...
        private bool IsIncluded(StringFilterCondition filterCondition, string filter, string entryToCheck)
        {
            ...
            switch (filterCondition)
            {
                ...
                {
                    if (entryToCheck.Contains(filter) == false) included = false;
        ...
        private bool IsIncluded(IntFilterCondition filterCondition, int? filter, string entryToCheck)
        {
            ...
            if (!int.TryParse(entryToCheck, out int intEntry) || !filter.HasValue) return included;
            switch (filterCondition)
            {
                case IntFilterCondition.Equal:
                {
                    if (filter != intEntry) included = false;
                    break;
                }
